export default {
  ...
  async function handleRootRequest(request, USERNAME, PASSWORD, enableAuth) {
    ...
    <html lang="zh-CN">
      <head>
        ...
        <meta name="keywords" content="Image Hosting,Workers图床, Cloudflare, Workers,telegra.ph, 图床">
        <title>Image Hosting-XKABCTT</title>
      ...
      <body>
        ...
        <div class="card">
          <div class="title">Image Hosting</div>
          <button type="button" class="btn" id="viewCacheBtn" title="History"><i class="fas fa-clock"></i></button>
          <div class="card-body">
            ...
            async function uploadFile(file) {
              try {
                ...
                if (!acceptedTypes.includes(file.type)) {
                  toastr.error('Only JPEG, PNG OR WebP format supported。');
                  return;
                }
                ...
                if (interfaceInfo.compressImage === true) {
                  ...
                  toastr.error('File(s) must be ≤' + interfaceInfo.otherMaxSize / (1024 * 1024) + 'MB');
                  return;
                }
                ...
                $('#fileLink').val('File processing failure！');
                toastr.error('File processing failure！');
              } finally {
                toastr.clear();
              }
            }
              ...
              return new Promise((resolve) => {
                ...
                image.onload = () => {
                  ...
                  canvas.toBlob((blob) => {
                    ...
                    toastr.success('Image compressed successfully！');
            ...
            $('#viewCacheBtn').on('click', function() {
              ...
              if (isCacheVisible) {
                ...
                if (cacheData.length > 0) {
                  ...
                  cacheContent.append('<div>No record yet！</div>').show();
                }
                isCacheVisible = true;
              }
            });
    ...
    const mediaHtml = mediaData.map(({ key, url, timestamp }) => {
      ...
      if (fileExtension === 'mp4') {
        ...
        <div class="media-container" data-key="${key}" onclick="toggleImageSelection(this)">
          ...
          <div class="upload-time">Upload time: ${new Date(timestamp).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}</div>
        </div>
        `;
      } else {
        return `
      ...
      <head>
        ...
        <script>
          ...
          async function deleteSelectedImages() {
            ...
            if (response.ok) {
              alert('Selected media has been deleted');
              location.reload();
            } else {
      ...
      <body>
        <div class="header">
          <div class="header-left">
            <span>Total of ${mediaData.length} media file(s)</span>
          </div>
          <div class="header-right hidden">
            <span>Selected: <span id="selected-count">0</span></span>
            <button id="delete-button" class="delete-button" onclick="deleteSelectedImages()">Delete Selection</button>
          </div>
        </div>
        <div class="gallery">
          ${mediaHtml}
  ...
  async function handleUploadRequest(request, DATABASE, enableAuth, USERNAME, PASSWORD, domain, TG_BOT_TOKEN, TG_CHAT_ID) {
    try {
      ...
      if (!file) throw new Error('Missing File(s)');
      ...
      if (!telegramResponse.ok) throw new Error('Failed to Upload to Telegram');
      ...
      if (!filePathResponse.ok) throw new Error('Failed to get file path');
  ...
  async function handleDeleteImagesRequest(request, DATABASE) {
    ...
    try {
      ...
      if (keysToDelete.length === 0) {
        return new Response(JSON.stringify({ message: 'No items to delete' }), { status: 400 });
      }
      const placeholders = keysToDelete.map(() => '?').join(',');
      await DATABASE.prepare(`DELETE FROM media WHERE file_path IN (${placeholders})`).bind(...keysToDelete).run();
      return new Response(JSON.stringify({ message: 'Deleted successfully' }), { status: 200 });
    } catch (error) {
      console.error('Error when deleting image(s):', error);
